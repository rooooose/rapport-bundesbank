...
Puis, une fois la page créée grâce à un fichier Twig (front-end) et une fonction dans le controleur indiquant la route vers cette page (back-end), il fallait afficher les informations de la base de données dans un tableau html, en passant par javascript.\\
Pour ce faire, il fallait tout d'abord récupérer l'ensemble des informations de chaque utilisateur via le controleur. J'ai donc fait une requete Doctrine via le Repository de l'entité Utilisateur.


difficulté : serialiser les données en json, surtout tableaux de tableaux (possessions)



\subsubsection{Cartes des entreprises}

Récupération des entreprises appartenant à une sous catégories : ajout dans la table "product" d'un attribut company\_id car l'attribut initial "owner" renvoyait à un individu de l'entité "user".
